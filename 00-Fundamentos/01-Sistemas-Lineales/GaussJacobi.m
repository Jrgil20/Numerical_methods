function [tabla_resultados, solucion] = GaussJacobi(matriz_coeficientes, vector_independientes, valores_iniciales, num_iteraciones_max, error_tolerancia)
...
%DESCRIPCION: Resuelve un sistema de ecuaciones lineales usando el método de Gauss-Jacobi
%con pivoteo parcial para mejorar la convergencia.
...
if nargin < 5                 
    error('Insuficientes datos de entrada. Se necesitan 5 parámetros');
else                          
...
% DIFERENCIA CON GAUSS-SEIDEL: En Jacobi se usan los valores de la iteración
% anterior para calcular todos los nuevos valores (no se actualizan dentro de la iteración)
...
for iteracion = 1:num_iteraciones_max - 1
    % Guardar valores de la iteración anterior
    ...
    for variable = 1:num_variables
        ...
        % Fórmula de Gauss-Jacobi (usa TODOS los valores de la iteración anterior):
        % x_i^(k+1) = (b_i - suma(a_ij * x_j^(k))) / a_ii
        valores_iteracion(iteracion + 1, variable) = (vector_independientes(variable) - sum(matriz_coeficientes(variable, otras_variables) .* valores_anterior(otras_variables))) / matriz_coeficientes(variable, variable);
    end
